      ...
      searchElements.forEach(function (item) {
        ...
        index.search(query, searchSettings).then(({ hits }) => {
          ...
          hits.forEach(function (hit){
            ...
            newNode.innerHTML = '<div class="amp-dc-blog-item"><a href="javascript:void(0);" onclick="AmpCa.Utils.evaluateAmplienceLink(' + hit._meta.deliveryKey + ', "page");"><img class="amp-dc-blog-image" src="//' + hit.image.image.defaultHost + '/i/' + hit.image.image.endpoint + '/' + hit.image.image.name + '?&sm=c&h=140&w=414&scaleFit=poi&poi={$this.metadata.pointOfInterest.x},{$this.metadata.pointOfInterest.y},{$this.metadata.pointOfInterest.w},{$this.metadata.pointOfInterest.h}" alt="' + hit.image.altText + '"> <h1 class="amp-dc-blog-title">' + hit.title + '</h1><div class="amp-dc-blog-description">' + hit.description + '</div></a></div>'
